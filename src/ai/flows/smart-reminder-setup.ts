...
/**
 * @fileOverview This file defines a Genkit flow for setting up smart reminders for permit and insurance expiry dates.
 *
...
const SmartReminderSetupInputSchema = z.object({
  prompt: z
    .string()
    .describe(
      'A prompt containing the details for setting up reminders, including permit and insurance expiry dates.'
    ),
});
...
const SmartReminderSetupOutputSchema = z.object({
  type: z.enum(["Vehicle Permit", "Insurance"]).optional().describe("The type of the reminder."),
  ...
  relatedTo: z.string().optional().describe("A vehicle number this reminder is related to."),
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a virtual assistant that helps business owners set up reminders for important dates such as vehicle permits and insurance expiry.

  Based on the provided prompt, extract the necessary information and return it in the output schema.
  - The 'type' should be one of "Vehicle Permit" or "Insurance".
  ...
  - The 'relatedTo' should contain a vehicle registration number if mentioned.
  ...
  Output: { success: false, message: "This type of reminder is not supported. Please use reminders for Vehicle Permits or Insurance." }
  `,
});
